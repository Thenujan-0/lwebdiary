...
var diaryNamesBar = {
  ...
  overflowing: function overflowing() {
    return bar.prop("scrollWidth") - bar.width() > 5;
  },
  overflowWidth: function overflowWidth() {
  ...
  handleScrollButtons: function handleScrollButtons() {
    if (diaryNamesBar.hiddenElements.length > 0) {
      diaryNamesBar.scrollButtons.each(function () {
    ...
    if (diaryNamesBar.overflowing()) {
      ...
      while (diaryNamesBar.overflowing() && diaryNamesBar.getElems().length > 1) {
        ...
        diaryNamesBar.hiddenElements.push({
          elem: lastElem,
          width: lastElem.width() + parseInt(lastElem.css("padding-left")) + parseInt(lastElem.css("padding-right"))
        });
        lastElem.remove();
      }
      ...
      var hidden = diaryNamesBar.hiddenElements;
      ...
      while (diaryNamesBar.remainingWidth() > elemWidth && hidden.length > 0) {
        console.log("appending item"); // $("div.diaryNames").append(hidden[hidden.length-1].elem)
  ...
  appendItem: function appendItem(elem) {
  ...
  onresize: function onresize() {
    if (!diaryNamesBar.resizeTimeout) {
      ...
      diaryNamesBar.resizeTimeout = setTimeout(function () {
        diaryNamesBar.resizeTimeout = null;
      }, 100);
